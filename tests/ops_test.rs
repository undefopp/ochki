...
async fn test_dump_load_roundtrip() {
    ...
    ochk::commands::create::run(&f.client, "/dump/a", Some("leaf"), false, false, true)
        .await
        .expect("setup failed");

    let json = ochk::commands::dump::run(&f.client, "/dump")
        .await
        .expect("dump failed");
    ...
    let r = ochk::commands::load::run(&f.client, "/restored", tmp.path().to_str().unwrap(), false)
        .await
        .expect("load failed");
    assert!(r.contains("2 nodes"));

    let r = ochk::commands::get::run(&f.client, "/restored/dump/a")
        .await
...
async fn test_acl_roundtrip() {
    let f = ZkFixture::setup().await;
    ochk::commands::create::run(&f.client, "/acl-test", None, false, false, false)
        .await
        .expect("setup failed");

    let r = ochk::commands::acl::get_acl(&f.client, "/acl-test")
        .await
    ...
    ochk::commands::acl::set_acl(&f.client, "/acl-test", "world:anyone:r", None)
        .await
...
async fn test_diff_same_cluster() {
    let f = ZkFixture::setup().await;
    ochk::commands::create::run(&f.client, "/diff-test", Some("same"), false, false, false)
        .await
        .expect("setup failed");
    ...
    let r = ochk::commands::diff::run(&f.client, &c2, "/diff-test")
        .await
        .expect("diff failed");
...
async fn test_json_output() {
    let f = ZkFixture::setup().await;
    ochk::commands::create::run(&f.client, "/json-test", Some("data"), false, false, false)
        .await
        .expect("setup failed");

    let r = ochk::commands::get::run(&f.client, "/json-test")
        .await
        .expect("get failed");
    ...
    let r = ochk::commands::exists::run(&f.client, "/json-test", false)
        .await
...
async fn test_binary_data() {
    ...
    let r = ochk::commands::get::run(&f.client, "/bin")
        .await
...
async fn test_path_normalization() {
    let f = ZkFixture::setup().await;
    let r = ochk::commands::create::run(&f.client, "norm", None, false, false, false)
        .await
        .expect("create without slash failed");
    assert_eq!(r.path, "/norm");

    let r = ochk::commands::get::run(&f.client, "norm")
        .await
